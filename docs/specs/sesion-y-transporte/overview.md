...
## Flujo: cierre inesperado de la sesión (el proceso real muere solo)

`resolve_claude_executable` (backend) solo confirma que el binario `claude` existe en el `PATH` — no que la persona ya corrió `claude login`. Si el binario existe pero no hay sesión iniciada en el CLI real, `cmd.spawn()` tiene éxito igual: el comando `start_claude_session` devuelve `Ok(())`, `App.vue` pone `sessionActive = true` y muestra la interfaz de chat normal, sin ningún indicio de que el proceso real vaya a fallar. El fallo real (típicamente el CLI imprimiendo el error de autenticación por `stderr` y cerrándose) llega después, de forma asíncrona, y **antes de esta corrección no tenía ningún sitio de captura en el frontend**: `onStderr`/`onSessionClosed` (`src/claude-transport.ts`) eran wrappers ya escritos pero sin ningún llamador real — un mensaje del usuario enviado a un proceso ya muerto fallaba en silencio (`sendChatMessage`, `catch` vacío salvo resetear `isSending`).

Ahora `App.vue` escucha ambos eventos desde el montaje:
- `onStderr` solo guarda la última línea en `lastStderrLine` (variable de módulo, no reactiva, reseteada a `null` al arrancar o reanudar una sesión) — no se muestra cada línea suelta como error, `onStderr` también dispara para fallos best-effort no fatales (ver "Persistencia de última sesión" abajo).
- `onSessionClosed` es el disparador real: si el cierre fue esperado (`expected: true`, el caso de `closeActiveSession`) no hace nada. Si fue inesperado con una sesión que la app creía activa, arma el mensaje con `buildUnexpectedCloseMessage` (`src/session-closed-notice.ts`, función pura) — prioriza la última línea de stderr real si la hay (suele traer la razón concreta, p.ej. el CLI reportando falta de autenticación), si no cae al código de salida, si no hay ninguno de los dos deja un mensaje genérico — pone `sessionActive = false`, `sessionId = null`, muestra el mensaje en el mismo banner de error que el caso "Claude Code no instalado" y lo registra en el log de fallas (`registerFailure('process-error', ...)`).

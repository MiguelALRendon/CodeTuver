...
- `src-tauri/tauri.conf.json`: `bundle.createUpdaterArtifacts: true`; `plugins.updater.pubkey` (llave pública Ed25519 real del proyecto, regenerada para `v0.1.0`); `plugins.updater.endpoints` = `["https://github.com/MiguelALRendon/CodeTuver/releases/latest/download/latest.json"]` — verificado en vivo: resuelve un `latest.json` real servido desde el release `v0.1.0`.
- `npm run tauri build` firma los artefactos del updater automáticamente si `TAURI_SIGNING_PRIVATE_KEY` (ruta o contenido de la llave privada) está en el entorno; si la llave tiene contraseña, también hace falta `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` — el CLI de Tauri siempre imprime "Info Decrypting updater signing key, expect a prompt for password" y **intenta leer una contraseña por stdin en ese paso incluso con una llave sin contraseña**; en un entorno no interactivo (`stdin` cerrado) esto se queda esperando para siempre si `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` no está seteada explícitamente (aunque sea a cadena vacía). Genera dos `.sig` (uno por instalador, MSI y NSIS) junto a cada bundle en `src-tauri/target/release/bundle/`.
- `latest.json` (subido a mano como asset del release, no generado por CI todavía): `{ version, notes, pub_date, platforms: { "windows-x86_64": { signature, url } } }` — `signature` es el contenido íntegro del `.sig` del instalador NSIS (`*-setup.exe.sig`), `url` apunta al asset del instalador NSIS del mismo release.
